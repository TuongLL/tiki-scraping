...
def process_product_by_link(info: dict):
    ...
    print('Crawling: ...',info['query_value'])
    print(f'https://tiki.vn/api/v2/categories/{info["query_value"]}')
    response = requests.get(f'https://tiki.vn/api/v2/categories/{info["query_value"]}?include=ancestors', headers ={
...
if __name__ == "__main__":
    df_link = pd.read_csv(
        os.path.join(os.getcwd(), "link_category2.csv"), delimiter=','
       )
